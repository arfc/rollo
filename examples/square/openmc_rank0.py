import sys
# pretend we are serial Python to avoid loading parallel-hdf5-h5py which call MPID_Init
for i in range(len(sys.path)):
  sys.path[i] = sys.path[i].replace("/mnt/bwpy/mpi/", "/mnt/bwpy/single/")
import openmc
import numpy as np
...
if rank == 0:
    # Templating 
    total_pf = 0.07125728463910587
    poly_coeff = [0.2838161903172449, 0.7588093594552487, 0.37835915519985175, 0.07674254577140349] 
    # Constants
    T_r1 = 2135e-5
    T_r2 = 3135e-5
    T_r3 = 3485e-5
    T_r4 = 3835e-5
    T_r5 = 4235e-5    uoc_9 = openmc.Material()
    ...
    por_c.temperature = 948    si_c = openmc.Material()
    ...
    si_c.temperature = 948    graphite = openmc.Material()
    graphite.set_density('g/cc', 1.8)
    graphite.add_nuclide('C0', 9.025164e-2)
    graphite.temperature = 948    lm_graphite = openmc.Material()
    lm_graphite.set_density('g/cc', 1.8)
    lm_graphite.add_nuclide('C0', 9.025164e-2)
    lm_graphite.temperature = 948    flibe = openmc.Material()
    ...
    flibe.temperature = 948    mats = openmc.Materials(
        (uoc_9,
         por_c,
         si_c,
         graphite,
         lm_graphite,
         flibe))    spheres = [openmc.Sphere(r=r)
    ...
    triso_univ = openmc.Universe(cells=triso_cells)    mats.export_to_xml()    # shape of reactor
...
#openmc.run(threads=16, mpi_args=["mpiexec", "-n", "-32"])
openmc.run()
